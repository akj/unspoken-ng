...
def test_the_audio_group_is_not_in_ci():
    """CI runners have no audio device; installing soundcard there is noise."""
    for name in _group_pins("audio"):
        assert name not in _ci_pins()
    for spec in GROUPS["audio"]:
        package = re.split(r"[<>=!~\[]", spec)[0].strip().lower()
        assert package not in CI.lower(), (
            f"{package} appears in ci.yml; the audio group is for local rigs only"
        )
